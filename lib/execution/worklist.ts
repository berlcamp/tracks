// What the Budget Office does next with a PPA, and how both execution screens
// band their rows by office.
//
...
/** The band fields the execution screens group on. */
...
/**
 * The execution rows, banded by office, with what that office has moved.
 *
 * Both screens read office by office: a clerk works one department's OBRs at a
 * time and rings that office about them, and the monitoring report is read by
 * the office whose utilisation it states. The rows arrive sorted by sector then department, so the groups
 ...
 * both screens mark such a subtotal "(filtered rows only)" exactly as the AIP
 * grid does.
 *
 ...
 * allotment total per OFFICE (v_aip_totals is per document, and the worklist
 * spans the annual AIP, its supplementals and every fund at once), and none of
 * it is ever printed. Each row's figures still come from v_ppa_financials
 * unchanged.
 */
export function groupByDepartment<T extends DepartmentFields & MoneyFields>(
  ...
  for (const row of rows) {
    ...
    group.totals.disbursed += Number(row.disbursed)
    group.totals.unobligated += Number(row.unobligated_balance)
...
/**
 * An office's obligation rate, on the same terms as one row's own: obligated
 * over allotted, rounded to two places, and NULL where nothing has been
 * released rather than 0%.
 *
 * v_ppa_financials states it that way per row (0004_views.sql) and monitoring
 * shows a dash for it, so a subtotal stating it any other way would not be the
 * same measure sitting under a column of them.
 */
export function obligationRate(totals: { allotted: number; obligated: number }): string | null {
  if (totals.allotted <= 0) return null
  return `${((totals.obligated / totals.allotted) * 100).toFixed(2)}%`
}
