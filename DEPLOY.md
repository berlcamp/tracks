...
| `0017_presentation_reports.sql` | **Apply after 0016.** The City Planning presentation deck: `tracks.presentation_deck()` and the views it aggregates. Read-only and additive — it adds no table, writes no row and changes no lock. |
| `0018_row_history.sql` | **Apply after 0017.** The audit trail on a PPA row, completed and made readable. Additive: it adds a nullable `ppa_revisions.changed_role`, replaces the revision trigger function to stamp it, and creates `v_ppa_revisions`. **No lock changes** — City Planning could already edit any office's row while the period is `open` or `consolidating`, and still cannot once it has gone to the LDC. What changes on screen is that the Consolidated AIP now offers Edit on the rows that lock already allowed, and every row on both grids offers History. Revisions recorded before this migration keep a null `changed_role`, and the panel says so rather than guessing from the role the account holds today. |
| `0019_demo_mode.sql` | **Apply after 0018.** Demo mode — Settings → Demo. Additive: `lgu_settings.demo_mode`, `aip_periods.is_demo`, and the functions behind the switch. It seeds nothing on apply; the demo year is built the first time an administrator turns the switch on, and it lands on the newest free year at or below 2025, so **check you have no real period in 2015–2025 you would rather keep free.** It replaces four policies — `aip_periods_read`, `aip_periods_admin_write`, `aips_read`, `aips_planning_write` — adding a visibility predicate to each; with demo mode off (the default) all four behave exactly as before, because `is_demo` is false on every existing period. No demo sign-in is created and no service-role key is needed. |
